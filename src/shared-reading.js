...
async function init() {
  ...
  if (!sharedRecord) {
    throw new Error("공유된 사주 정보를 찾지 못했습니다.");
  }
  ...
  document.title = `${sharedRecord.entry_name} 사주 공유 | 소울 스캔`;
  $("shareHeading").textContent = `${sharedRecord.entry_name}님의 공유 사주`;
  $("shareSubtitle").textContent = "공유 링크로 열람 중인 페이지입니다. 로그인하지 않아도 결과를 그대로 볼 수 있습니다.";
  $("shareSection").classList.remove("hidden");
}

init().catch((error) => {
  $("shareError").textContent = error.message || "공유된 사주 페이지를 불러오지 못했습니다.";
});
